...
exports.handler = async (event, context) => {
    ...
    if (clientIp && clientIp !== 'N/A' && clientIp !== '127.0.0.1') { // Avoid local/undefined IPs for GeoIP
        try {
            console.log(`[Netlify initiate-dns-test] Client connection IP from Netlify header: ${clientIp}`);
            // Using ip-api.com for GeoIP for the client's IP
            const geoIpApiUrl = `http://ip-api.com/json/${clientIp}?fields=status,message,countryCode,country,city,isp,org,query`;
            ...
            if (geoIpResponse.ok) {
                ...
                if (geoIpData.status === 'success') {
                    clientIpInfo = {
                     ...
                     clientIpInfo.isp = `GeoIP lookup error: ${geoIpData.message || 'Failed status'}`; // Store error in isp field
                }
            } else {
                console.warn(`[Netlify initiate-dns-test] ip-api.com HTTP error for client IP ${clientIp}: ${geoIpResponse.status}`);
                clientIpInfo.isp = `GeoIP lookup HTTP error: ${geoIpResponse.status}`; // Store error in isp field
            }
        } catch (e) {
            console.error("[Netlify initiate-dns-test] Error fetching client IP GeoIP details:", e.message);
            clientIpInfo.isp = `GeoIP fetch error`; // Store error in isp field
        }
    } else {
        console.log("[Netlify initiate-dns-test] Client IP not available or is local, skipping GeoIP lookup. IP was:", clientIp);
        if (clientIpInfo.ip === 'N/A' || clientIpInfo.ip === '127.0.0.1') {
            clientIpInfo.isp = "Could not determine from headers";
        }
    }
    // --- End of Client IP logic ---

    // --- Main test logic (leak_id and hostnames_to_ping generation) ---
    try {
        ...
        return {
            ...
            body: JSON.stringify({ 
                leak_id: leak_id, 
                hostnames_to_ping,
                your_public_ip_information: clientIpInfo // Send the determined client IP info
            })
        };
    } catch (error) {
